function [dets, boxes, info, model_score] = gdetect(pyra, model, thresh, bbox, overlap)
...
% assign the model_score
model_score.padx = pyra.padx;
model_score.pady = pyra.pady;
model_score.scales = pyra.scales;
model_score.symbols(model.start).score = model.symbols(model.start).score;
model_score.rules{model.start} = model.rules{model.start};
